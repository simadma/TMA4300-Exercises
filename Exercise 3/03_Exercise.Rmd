...
Seen in Algorithm \ref{alg:c-bootstrap}.
...
param_boot <- function(lambda_hat, B = 10000, itermax = 300, tol = 1e-5) {
  ...
  for (i in 1:B) {
    x_star <- rexp(n, lambda_hat[1])
    y_star <- rexp(n, lambda_hat[2])
    z_star <- pmax(x_star, y_star)
    u_star <- 1*(x_star >= y_star)
    lambda_star <- EM(z_star, u_star, lambda_hat, itermax, tol)$lambda
    lambda_list[i,] <- lambda_star
  }
  sigma_hat <- apply(lambda_list, 2, sd)
  mu_hat <- apply(lambda_list, 2, mean)
  bias <- mu_hat - lambda_hat
  rho_hat <- cor(lambda_list[, 1], lambda_list[, 2])
