...
$$
such that we need to find the lower triangular matrix $A$ such that $\Sigma = A A^\top$, and this is done using `chol()` in `R`. We construct the function `d_variate_normal()` in the following code block to simulate using the Box-Muller algorithm.
...
$$
  \frac{\mathrm{d} f(\theta \mid \bfy)}{\mathrm{d} \theta} \propto \frac{\mathrm{d} f^*(\theta \mid \bfy)}{\mathrm{d} \theta} = (\theta-1)^{37} \theta^{33} (\theta + 2)^{124} (197 \theta^2 - 15 \theta - 68) = 0.
$$
...
$$
such that $k = f(\theta_\mathrm{max} \mid \bfy) / K$. This however is found numerically with the `optimize()`-function in `R` below. We then simulate $\Theta \sim \Uniform(0, 1)$ and $U \sim \Uniform(0, 1)$ and calculate $\alpha = f(\Theta \mid \bfy) / k$. Then, if $U \leq \alpha$, $\Theta$ is returned, and if not, the procedure is run again. We then sample from the posterior distribution in the code block below.
...
mu_num
```
From this it is clear that the estimated posterior mean is $\hat{\mu} \approx `r round(mu_est, 3)`$ using Monte Carlo integration, and $\mu \approx `r round(mu_num, 3)`$ using numerical integration with `integrate()`. Figure \ref{fig:Monte_Carlo} also shows that these means corresponds well to the real posterior mean.
