  ...
  if (n_iter > 1) {
    for (i in 2:n_iter) {
      ...
      param <- MH_t1_lamb0_lamb1(
                 t_1, lambda_0, lambda_1, beta, y_0, y_1, y, x, t_0, t_2, d
               )
      t_1 <- param$t_1
      y_0 <- param$y_0
      y_1 <- param$y_1
...
MH_beta_lamb0_lamb1 <- function(t_1, lambda_0, lambda_1, beta, y_0, y_1, t_0, t_2, d) {
  ...
  list(beta = beta, lambda_0 = lambda_0, lambda_1 = lambda_1)
}
...
# lambda_1: initial rateparameter
# beta:     initial hyperparameter
# d1:       tuning parameter for the proposal distribution of t_1
# d2:       tuning parameter for the proposal distribution of beta
# n_iter:   number of iterations
block_McMC_2 <- function(
                  data, t_1, lambda_0, lambda_1, beta, d1 = 1, d2 = 1, n_iter = 5000
                ) {
  x <- data$date
  y <- length(x)  # Number of disasters
  t_0 <- x[1]     # Start date
  ...
  if (n_iter > 1) {
    for (i in 2:n_iter) {
      # Metropolis Hastings step, sample t_1
      param <- MH_t1(t_1, lambda_0, lambda_1, t_0, t_2, x, y_0, y_1, y, d1)
      t_1 <- param$t_1
      y_0 <- param$y_0
      y_1 <- param$y_1
      
      # Metropolis Hastings step, sample (beta, lambda_0, lambda_1)
      param2 <- MH_beta_lamb0_lamb1(t_1, lambda_0, lambda_1, beta, y_0, y_1, t_0, t_2, d2)
      beta <- param2$beta
...
## Subproblem 8.
```{r block1 sample, fig.cap="\\label{fig:A-block1}......"}
set.seed(93)
n_iter <- 5000
d <- c(1, 10, 200)

# Sample 3 chains with different values for tuning parameter
theta5 <- block_McMC_1(coal, coal$date[nrow(coal)], 10, 10, 10, d[1], n_iter)
theta6 <- block_McMC_1(coal, coal$date[nrow(coal)], 10, 10, 10, d[2], n_iter)
theta7 <- block_McMC_1(coal, coal$date[nrow(coal)], 10, 10, 10, d[3], n_iter)

# Plot trace
plot_trace(theta5, theta6, theta7) + scale_color_hue(name = "d", labels = d)
```

```{r block1 burnin, fig.cap="\\label{fig:A-block1-burnin}......"}
burnin <- 4000
...
```{r block2 sample, fig.cap="\\label{fig:A-block2}......"}
set.seed(12)

# Sample chain with different values of tuning parameter
theta8 <- block_McMC_2(coal, coal$date[nrow(coal)], 10, 10, 10, 10, d[1], n_iter)
theta9 <- block_McMC_2(coal, coal$date[nrow(coal)], 10, 10, 10, 10, d[2], n_iter)
theta10 <- block_McMC_2(coal, coal$date[nrow(coal)], 10, 10, 10, 10, d[3], n_iter)

# Plot trace
plot_trace(theta8, theta9, theta10) + scale_color_hue(name = "d", labels = d)
```

```{r block2 burnin, fig.cap="\\label{fig:A-block2-burnin}......"}
burnin <- 1000
...
```{r theta marginal densities, fig.cap="\\label{fig:theta-marginal}"}
