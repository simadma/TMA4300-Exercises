...
\begin{split}
  ...
  [\lambda_1\mid t_1,\lambda_0,\beta,x]
    ...
    &\propto f_{\theta\mid x}(\theta\mid x) \\
    &\propto I_{[0,\infty)}(\lambda_1)\cdot \lambda_1^{(y_1 + 2) - 1}\exp\left\{-\left(t_2 - t_1 + \frac{1}{\beta}\right)\lambda_1\right\} \\
...
# Performs a Metropolis Hastings step to sample t_1
...
# Performs a Metropolis Hastings step to sample (t_1, lambda_0, lambda_1)
...
# Performs a Metropolis Hastings step to sample (beta, lambda_0, lambda_1)
...
Figure \ref{fig:A-block1} shows a nine chains with combinations of different tuning parameters for $\beta$ in the first proposal block, and for $t_1$ in the second proposal block.
We see that three of the chains did not converge.
We will eliminate these, together with the chains that had an acceptance probability less than $15\%$, which leaves the second chain with $(d_t, t_\beta) = (10, 1)$ with a burn-in period of about $n=500$.

```{r block1 sample, fig.cap="\\label{fig:A-block1}Trace plots of nine chains with blocking proposals with different tuning parameter values"}
set.seed(93)
n_iter <- 5000
d1 <- c(1, 10, 200)
d2 <- c(1, 2, 10)
d_grid <- expand.grid(d1, d2)
lab <- with(d_grid, paste0("(", Var1, ", ", Var2, ")"))

# Sample 3 chains with different values for tuning parameter
theta5 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[1], d2[1], n_iter)
theta6 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[2], d2[1], n_iter)
theta7 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[3], d2[1], n_iter)
theta8 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[1], d2[2], n_iter)
theta9 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[2], d2[2], n_iter)
theta10 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[3], d2[2], n_iter)
theta11 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[1], d2[3], n_iter)
theta12 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[2], d2[3], n_iter)
theta13 <- block_McMC(coal, coal$date[nrow(coal)], 10, 10, 10, d1[3], d2[3], n_iter)

# Plot trace
plot_trace(theta5, theta6, theta7, theta8, theta9, theta10, theta11, theta12, theta13) +
  scale_color_hue(name = "(d_t, d_beta)", labels = lab)
```

We now compare the single-site chain with tuning parameter $d=10$ and our candidate from the block proposal chain. We see that the single-site chain explores more of the domain.
A possibility is that there is a computational error in the acceptance probability for the block proposal.
However, they both yield a quite good mixing, but not perfect, as $t_1$ looks a bit sparse.

```{r block1 burnin, fig.cap="\\label{fig:A-block1-burnin}The best candidate of the chains with blocking proposals and the single-site proposal chain."}
burnin <- 500
theta6 <- tail(theta6, -burnin)

theta1 <- tail(theta1, nrow(theta6))

# Plot trace
plot_trace(theta1, theta6) + scale_color_hue(labels = c("Single-site", "Block"))
```

We use the single-site chain to estimate the marginal posterior density of each of the variables.
Figure \ref{fig:theta-marginal} shows that the expected change point is at $t_1=1890.66$, where the disasters per year is reduced from $\lambda_0=3.14$ to $\lambda_1=0.95$, which is a major improvement for the coal miners, although still not ideal.
...
The scatter plot of $[\lambda_0,\lambda_1\mid x]$ is shown in Figure \ref{fig:A-cov-lamb} also showing $\Cov[\lambda_0,\lambda_1\mid x]$. We see that there is a very small correlation.

```{r, fig.width=3.5, fig.asp=1, fig.cap="\\label{fig:A-cov-lamb}Scatter plot of $[\\lambda_0 \\mid x]$ and $[\\lambda_1 \\mid x]$ and their covariance."}
theta1 %>% 
