...
\begin{equation}\label{eq:A-posterior}
  f_{\theta\mid x,y}(\theta\mid x) = \frac{1}{c}\cdot \frac{\lambda_0^{y_0 + 1}\lambda_1^{y_1 + 1}}{\beta^5} \exp\left\{-\left(\lambda_0(t_1 - t_0) + \lambda_1(t_2 - t_1) + \frac{1}{\beta}(\lambda_0 + \lambda_1 + 1)\right)\right\},
\end{equation}
...
\begin{split}
  [t_1\mid\lambda_0,\lambda_1,\beta,x,y]
    ...
    &\propto I_{(t_0, t_2)}(t_1)\cdot\exp\left\{-(\lambda_0 - \lambda_1)t_1\right\} \\
    &\sim\text{Unknown distribution}; \\
  [\lambda_0\mid t_1,\lambda_1,\beta,x,y]
    &\sim f_{\lambda_0\mid t_1,\lambda_1,\beta,x,y}(\lambda_0\mid t_1,\lambda_1,\beta,x,y) \\
    &\propto f_{\theta\mid x,y}(\theta\mid,x,y) \\
  ...
  [\lambda_1\mid t_1,\lambda_0,\beta,x,y]
    &\sim f_{\lambda_1\mid t_1,\lambda_0,\beta,x,y}(\lambda_1\mid t_1,\lambda_0,\beta,x,y) \\
    &\propto I_{[0,\infty)}(\lambda_1)\cdot f_{\theta\mid x,y}(\theta\mid,x,y) \\
...
## Subproblem 4.
We implement a single site McMC algorithm for $f(\theta\mid x,y)$, where we need to do a Gibbs step for all the full conditionals except for $[t_1\mid\lambda_0,\lambda_1,\beta,x,y]$, where we will do a Metropolis Hastings step.
Let $t = [t_1\mid\lambda_0,\lambda_1,\beta,x,y]$, and let $t^* = (t - t_0)/(t_2 - t_0)$ be the proposed values on the domain $(t_0, t_2)$ and $(0,1)$, respectively, and let $s$ be the current state of the Markov chain.
We draw $t^*$ from the Beta distribution with parameters $(ns^*, n(1 - s^*))$, where $n$ is a tuning parameter. That is, the proposal distribution $Q(t\mid s)$ is given by
$$
 Q(t\mid s) \propto \frac{1}{B(ns^*, n(1 - s^*))}(t^*)^{ns^* - 1}(1 - t^*)^{n(1 - s^*) - 1},\quad s^* = \frac{s - t_0}{t_2 - t_0},\quad t^* = \frac{t - t_0}{t_2 - t_0},
$$
and the target distribution is given by
$$
  \pi(t) \propto \exp\{-\lambda t\} = \exp\{-\lambda(t_0 + t^*(t_2 - t_0))\}\propto \exp\{-\lambda t^*(t_2 - t_0)\},
$$
where $\lambda = \lambda_0 - \lambda_1$.
The acceptance probability is given by
$$
\alpha(t\mid s) = \min\left\{1, \frac{\pi(t)\cdot Q(s\mid t)}{\pi(s)\cdot Q(t\mid s)}\right\}.
$$
We compute the ratio on log scale to avoid numerical instability.
$$
\begin{split}
 \log\frac{\pi(t)\cdot Q(s\mid t)}{\pi(s)\cdot Q(t\mid s)}
 &= \log\frac{\e^{-\lambda t^*(t_2 - t_0)}\cdot \frac{1}{B(nt^*, n(1 - t^*))}(s^*)^{nt^* - 1}(1 - s^*)^{n(1 - t^*) - 1}}{\e^{-\lambda s^*(t_2 - t_0)}\cdot \frac{1}{B(ns^*, n(1 - s^*))}(t^*)^{ns^* - 1}(1 - t^*)^{n(1 - s^*) - 1}} \\
 &= -\lambda t^*(t_2 - t_0) - \log B(nt^*, n(1 - t^*)) + (nt^* - 1)\log s^* + (n(1 - t^*) - 1)\log(1 - s^*)\\
 &\quad+\lambda s^*(t_2 - t_0) + \log B(ns^*, n(1 - s^*)) - (ns^* - 1)\log t^* - (n(1 - s^*) - 1)\log(1 - t^*).
\end{split}
$$
